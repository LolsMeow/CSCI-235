...
void LinkedList<T>::rotate(int k){
   //stops after k iterations is called
   if(k == 0){ 
      return; //stops rotation
   }
   //list size needs to be more than 1
   if(item_count_ >1){
      Node<T> *end_ptr = head_ptr_; //sets a new pointer to the first node
      //traverses the list to the second last node
      while(end_ptr->getNext() != nullptr){ 
         end_ptr = end_ptr->getNext();
      }
      end_ptr->setNext(head_ptr_); //points last node to first node
      head_ptr_ = end_ptr; //sets originally last node as head node
      getNodeAt(item_count_-1)->setNext(nullptr); //points originally second last node to null to indicate its the end of the list
      rotate(k-1); //recursive call
   }
}
...
void LinkedList<T>::invertRest(Node<T>* current_first_ptr){
   if((current_first_ptr->getNext())->getNext() == nullptr){               //Checks to see if the ptr two in front of the current is a nullptr
      head_ptr_ = current_first_ptr->getNext();                            //headptr is now the ptr in front 
      head_ptr_->setNext(current_first_ptr);                               //sets the headptr to the next ptr of the current ptr
      current_first_ptr->setNext(nullptr);                                 //sets the current ptr to nullptr
      return;
   }
   else{
      invertRest(current_first_ptr->getNext());                            //Recursive function to go through the list
      (current_first_ptr->getNext())->setNext(current_first_ptr);          //sets the the ptr in front to the current ptr
      current_first_ptr->setNext(nullptr);                                 //sets the current ptr to nullptr
   }
